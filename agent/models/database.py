...
def init_db() -> None:
    """Initialise database and add missing columns/tables if required."""
    try:
        conn = sqlite3.connect("user_data.db")
        c = conn.cursor()
        ...
        # Make sure created_at exists in credentials
        try:
            c.execute("SELECT created_at FROM credentials LIMIT 1")
        except sqlite3.OperationalError as e:
            if "no such column: created_at" in str(e):
                logger.info("Adding created_at column to credentials table…")
                c.execute(
        ...
        logger.info("Database tables: %s", [row[0] for row in c.fetchall()])

        conn.commit()
        conn.close()
    except Exception:
        logger.exception("Database initialisation error – resetting database")
        reset_database()
